document.addEventListener('DOMContentLoaded', () => {
        ...
        paginatedProducts.forEach((product) => {
            ...
            tr.innerHTML = `
                ...
                <td class="actions">
                    ...
                    <button class="btn btn-success" onclick="applyPromotion(${product.id})">Promoção</button>
                </td>
            `;
    ...
    // Função para aplicar a promoção
    window.applyPromotion = async (id) => {
        const transaction = db.transaction(['products'], 'readwrite');
        const objectStore = transaction.objectStore('products');
        ...
        request.onsuccess = (event) => {
            ...
            const newPrice = parseFloat(prompt(`Digite o novo preço promocional para ${product.name}:`, product.price));

            if (newPrice && newPrice < product.price) {
                product.originalPrice = product.price; // Salva o preço original
                product.price = newPrice; // Atualiza o preço do produto
                objectStore.put(product).onsuccess = () => {
                    // Atualiza a interface para exibir o preço riscado e o novo preço
                    const tr = document.querySelector(`#product-${id}`);
                    tr.querySelector('.product-price').innerHTML = `<del>${product.originalPrice}</del> <span class="text-danger">${product.price}</span>`;
                    logHistory('Promoção aplicada', product);
                };
            } else {
